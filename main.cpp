...
double PV(double future_value, double coupon_rate, double r, int N,double d)
{
    ...
    for(int i=0;i<=N;i++)
    {
        ...
        if (i==N) temp_PV += future_value*pow(1+r,-N-d);
    }
    return temp_PV;
}
...
double iterationYTM(double future_value,double coupon_rate,double price_value,int N,double d,double i0=0.00001, double i1= 10, const int MaxIter=50000, double epsilon = 1e-7)
{
    ...
    double final_yield=0.0,temp_yield, result;
...
//test of function with some outputs
int main()
{
    //Yield-to-Maturity procedure
    double coupon_rate, future_value, price_value;
    float d;
    int N;
    cout << endl;
    ...
    cout << "--------------------------------------------------------" << endl;
    cout << endl;
    cout << "Please enter Bond characteristics : " << endl;
    ...
    //Results
    cout << "--------------------------------------------------------" << endl;
    cout << "YTM = " << setprecision(2) << iterationYTM(future_value,coupon_rate,price_value,N,d) * 100 << "%" <<endl;

    //Summary
    cout << endl;
    cout <<"Finally, we have a coupon rate = "<< setprecision(3) << coupon_rate*100 <<", a coupon value = "<< coupon_rate*future_value <<", the next coupon payment in "<< d <<" years"<<", a future value = "<< future_value <<", a price value = "<< price_value  << endl <<"And your bond comes to maturity in "<< N << " years." << endl;
